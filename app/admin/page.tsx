...
export default function AdminDashboard() {
  ...
  const handleSaveProduct = async () => {
    ...
    if (p) {
      ...
      alert("System Updated Successfully."); setShowAddForm(false); setIsEditing(false); fetchProducts();
  ...
  if (loading && !isAdmin) return <div className="min-h-screen flex items-center justify-center font-medium text-slate-400">Loading Portal...</div>
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd] text-slate-900 pb-20">
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-sm">
            <div className="max-w-5xl mx-auto w-full bg-white shadow-2xl rounded-2xl overflow-hidden flex flex-col min-h-[80vh]">
                <div className="p-6 md:p-8 border-b flex justify-between items-center bg-white sticky top-0 z-10">
                    ...
                    <button onClick={() => setShowAddForm(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 transition-colors">✕</button>
                </div>

                <div className="flex gap-4 md:gap-8 overflow-x-auto px-8 py-4 border-b bg-slate-50 sticky top-[73px] z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {activeSubTab === 'basic' && (
                            <>
                                <div className="space-y-4">
                                    <div><label className={sectionLabel}>Product Name (English)</label><input className="input-modern" value={formData.name_en} onChange={e => setFormData({...formData, name_en: e.target.value})} placeholder="e.g. Myvi Performance Shock" /></div>
                                    <div><label className={sectionLabel}>Product Name (BM)</label><input className="input-modern" value={formData.name_bm} onChange={e => setFormData({...formData, name_bm: e.target.value})} placeholder="e.g. Myvi Performance Absorber" /></div>
                                ...
                                <div className="space-y-4">
                                    <div><label className={sectionLabel}>Main Image URL (ImgBB)</label><input className="input-modern" value={formData.image_url} onChange={e => setFormData({...formData, image_url: e.target.value})} /></div>
                                    <div><label className={sectionLabel}>Gallery URLs (One per line)</label><textarea className="input-modern h-32" value={formData.gallery_input} onChange={e => setFormData({...formData, gallery_input: e.target.value})} /></div>
                                </div>
                            </>
                        )}
                        {activeSubTab === 'spec' && (
                            <>
                                ...
                                <div className="space-y-4">
                                    <div><label className={sectionLabel}>Manufacturer</label><select className="input-modern" value={formData.product_brand_id || ''} onChange={e => setFormData({...formData, product_brand_id: e.target.value})}><option value="">Select Brand</option>{productBrands.map(b => <option key={b} value={b}>{b}</option>)}</select></div>
                                    <div><label className={sectionLabel}>Warranty Info</label><input className="input-modern" value={formData.spec_warranty} onChange={e => setFormData({...formData, spec_warranty: e.target.value})} /></div>
                                </div>
                            </>
                        )}
                        {activeSubTab === 'desc' && (
                        ...
                        {activeSubTab === 'sales' && (
                            <div className="md:col-span-2 space-y-8">
                                <div className="flex items-center gap-4 bg-slate-50 p-6 rounded-xl border">
                                    <span className="font-bold text-sm">Use Variations?</span>
                                    <button type="button" onClick={() => setHasVariations(!hasVariations)} className={`px-8 py-2 rounded-full text-[10px] font-bold transition-all ${hasVariations ? 'bg-brand-orange text-white' : 'bg-slate-200 text-slate-500'}`}>{hasVariations ? 'ENABLED' : 'DISABLED'}</button>
                                </div>
                                {hasVariations ? (
                                    <div className="overflow-x-auto border rounded-xl shadow-sm">
                                        <table className="w-full text-left text-sm">
                                            <thead className="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase">
                                                <tr><th className="p-4">Variant</th><th className="p-4">Retail RM</th><th className="p-4 text-[#e11d48]">Dealer RM</th><th className="p-4">Stock</th></tr>
                                            </thead>
                                            <tbody>
                                                {variantGrid.map((v, i) => (
                        ...
                        {activeSubTab === 'ship' && (
                            <div className="space-y-8">
                                <div><label className={sectionLabel}>Weight (KG)</label><input type="number" step="0.1" className="input-modern max-w-xs" value={formData.weight_kg} onChange={e => setFormData({...formData, weight_kg: Number(e.target.value)})} /></div>
                                <div className="grid grid-cols-3 gap-4">
                                    <div><label className={sectionLabel}>L (CM)</label><input type="number" className="input-modern" value={formData.length_cm} onChange={e => setFormData({...formData, length_cm: Number(e.target.value)})} /></div>
                                    <div><label className={sectionLabel}>W (CM)</label><input type="number" className="input-modern" value={formData.width_cm} onChange={e => setFormData({...formData, width_cm: Number(e.target.value)})} /></div>
                                    <div><label className={sectionLabel}>H (CM)</label><input type="number" className="input-modern" value={formData.height_cm} onChange={e => setFormData({...formData, height_cm: Number(e.target.value)})} /></div>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="p-6 md:p-8 bg-slate-50 border-t flex justify-end gap-4 sticky bottom-0 z-10">
                    <button onClick={() => {setShowAddForm(false); setIsEditing(false);}} className="text-slate-400 font-bold text-xs px-6">Cancel</button>
                    {activeSubTab !== 'ship' ? (
                        <button onClick={handleNext} disabled={!isTabComplete(activeSubTab)} className={`px-12 py-4 font-bold rounded-lg text-sm transition-all ${isTabComplete(activeSubTab) ? 'bg-brand-orange text-white shadow-lg' : 'bg-slate-200 text-slate-400'}`}>Next Step →</button>
                    ) : (
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-12">
        ...
        {activeTab === 'products' && (
          <div className="space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-2xl border shadow-sm gap-6">
                <div><h2 className="text-2xl font-bold text-slate-800 tracking-tight">Stock Management</h2><p className="text-slate-400 text-sm">{products.length} Professional Listings Live</p></div>
                <button onClick={() => { setShowAddForm(true); setIsEditing(false); setActiveSubTab('basic'); }} className="bg-brand-orange text-white px-10 py-4 font-bold rounded-xl shadow-lg shadow-orange-500/20 hover:-translate-y-0.5 transition-all text-sm">+ Add New Product</button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {products.map(p => (
                    <div key={p.id} className="bg-white border p-6 hover:shadow-xl transition-all rounded-2xl flex flex-col group">
                        <div className="flex gap-4 items-start mb-6">
                            ...
                            <div className="overflow-hidden">
                                ...
                                <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest">{p.brand_name || 'Generic'}</p>
                            </div>
                        </div>
                        <div className="mt-auto pt-6 border-t flex justify-between items-end">
                            <div><p className="text-[10px] text-slate-400 font-bold mb-1">MSRP</p><span className="text-slate-800 font-black text-lg">RM{p.price_b2c?.toFixed(2)}</span></div>
                            <div className="flex gap-3">
                                <button onClick={() => handleEditClick(p)} className="text-[10px] font-bold text-brand-orange hover:underline">Edit</button>
                                <button onClick={async () => { if(confirm("Remove?")){ await supabase.from('products').delete().eq('id', p.id); fetchProducts() }}} className="text-[10px] font-bold text-slate-300 hover:text-[#e11d48]">Delete</button>
                            </div>
                        </div>
                    </div>
                ))}
