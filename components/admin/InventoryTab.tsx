...
export default function InventoryTab() {
  ...
  const labelS = "text-[10px] font-bold text-slate-400 uppercase mb-2 block tracking-widest italic";
  const inputS = "w-full border border-slate-200 p-4 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:border-blue-600 transition-all bg-white";
  ...
  return (
    <div className="space-y-8 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-2">
        ...
        <button onClick={() => {setShowForm(true); setIsEditing(false); setFormData(initialForm); setActiveSubTab('basic');}} className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold text-xs shadow-xl shadow-blue-500/20 active:scale-95 transition-all uppercase tracking-widest">+ Add New Part</button>
      </div>
            ...
            <tbody className="divide-y divide-slate-50">
                {products.map(p => (
                    <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="px-8 py-6"><span className="font-bold text-blue-800 bg-blue-50 px-3 py-1.5 rounded-lg text-xs tracking-tighter uppercase italic leading-none">PRO-{p.id}</span></td>
                        ...
                        <td className="px-8 py-6 text-right"><button onClick={() => handleEditClick(p)} className="text-slate-300 hover:text-blue-600 transition-all p-2 rounded-lg hover:bg-blue-50"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4Z"/></svg></button></td>
                    </tr>
                ))}
            </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-md">
            <div className="max-w-6xl mx-auto w-full bg-white shadow-2xl rounded-3xl overflow-hidden flex flex-col min-h-[85vh]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        {activeSubTab === 'basic' && ( <>
                            <div className="space-y-6">
                                ...
                                <div><label className={labelS}>Category</label><select className={inputS} value={formData.category_id || ''} onChange={e => setFormData({...formData, category_id: e.target.value})}><option value="">Select Category</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                            </div>
                            <div className="space-y-6">
                        ...
                        {activeSubTab === 'desc' && ( <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        {activeSubTab === 'sales' && ( <div className="md:col-span-2 space-y-10">
                                <button type="button" onClick={() => setHasVariations(!hasVariations)} className={`px-12 py-4 font-black text-xs italic transition-all ${hasVariations ? 'bg-[#f97316] text-white shadow-lg shadow-orange-500/20' : 'bg-slate-200 text-slate-500'}`}>{hasVariations ? 'VARIATIONS ENABLED' : 'ENABLE VARIATIONS'}</button>
                                        ...
                                        {!isEditing && variationLevels.map((level, lIdx) => (
                                            <div key={lIdx} className="bg-slate-50 border border-slate-200 p-8 relative rounded-lg">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                    <div><label className={labelS}>Level {lIdx+1} Name</label><input className={inputS} value={level.name} onChange={e => {const n = [...variationLevels]; n[lIdx].name = e.target.value.toUpperCase(); setVariationLevels(n);}} /></div>
                                                    <div><label className={labelS}>Options</label><input className={inputS} value={level.options.join(',')} onChange={e => {const n = [...variationLevels]; n[lIdx].options = e.target.value.split(',').map(s => s.toUpperCase()); setVariationLevels(n);}} /></div>
                                                </div>
                                            </div>
                                        ))}
                                                ...
                                                <tbody>
                                                    {variantGrid.map((v, i) => (
                                                        <tr key={i} className={`border-b border-slate-100 ${!v.is_active ? 'opacity-30' : 'hover:bg-slate-50'}`}>
                                                            ...
                                                            <td className="p-2"><input type="number" step="0.01" className="bg-white border border-slate-200 p-4 w-full text-sm font-black rounded-sm outline-none" value={variantGrid[i].price_b2c} onChange={e => { const g = [...variantGrid]; g[i].price_b2c = Number(e.target.value); setVariantGrid(g); }} /></td>
                                                            <td className="p-2"><input type="number" step="0.01" className="bg-white border border-slate-200 p-4 w-full text-sm font-black rounded-sm outline-none text-[#e11d48]" value={variantGrid[i].price_b2b} onChange={e => { const g = [...variantGrid]; g[i].price_b2b = Number(e.target.value); setVariantGrid(g); }} /></td>
                                                            ...
                                                            <td className="p-3 text-center"><button type="button" onClick={() => {const g = [...variantGrid]; g[i].is_active = !g[i].is_active; setVariantGrid(g);}} className={`px-5 py-2.5 text-[8px] font-black rounded-full transition-all ${v.is_active ? 'bg-green-600 text-white shadow-lg' : 'bg-slate-200 text-slate-400'}`}>{v.is_active ? 'ON' : 'OFF'}</button></td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                ...
                <div className="p-10 border-t bg-slate-50 flex flex-col md:flex-row justify-end gap-6 mt-auto sticky bottom-0 z-30">
                    <button onClick={() => {setShowForm(false); setIsEditing(false);}} className="px-10 py-5 font-black uppercase text-xs text-slate-400 italic">Exit Wizard</button>
                    {activeSubTab !== 'ship' ? (
                        <button onClick={() => setActiveSubTab(subTabOrder[subTabOrder.indexOf(activeSubTab) + 1])} className={`px-24 py-6 font-black uppercase italic text-sm tracking-widest transition-all rounded-md shadow-xl bg-blue-600 text-white hover:bg-[#0f172a]`}>Next Step →</button>
                    ) : (
                        <button onClick={handleSave} className={`px-24 py-6 font-black uppercase italic text-sm tracking-widest transition-all rounded-md shadow-xl bg-[#0f172a] text-white hover:bg-[#f97316]`}>{isEditing ? 'COMMIT UPDATES' : 'PUBLISH LISTING'}</button>
                    )}
                </div>
            </div>
        </div>
