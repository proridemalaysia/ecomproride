...
export default function CheckoutPage() {
  ...
  // 4. TOTALS CALCULATION (Using Number() to prevent NaN)
  ...
  const updateLogistics = useCallback(() => {
    if (address.postcode.length === 5 && cart.length > 0) {
      ...
      // Calculate Weight & Shipping (Highest of Actual vs Volumetric)
      const result = calculateShipping(address.postcode, cart)
      setShippingData(result)
      
      // Default to cheapest courier if not selected or invalid
      if (result.options.length > 0) {
        setSelectedCourier(result.options[0])
      }
    } else {
  ...
  const handlePlaceOrder = async () => {
    if (!customer.name || !customer.whatsapp || !customer.email || !selectedCourier || !address.state) {
      alert('Sila lengkapkan semua maklumat perhubungan dan penghantaran.'); 
      return;
    }
  ...
  const inputStyle = "w-full bg-white border border-slate-200 p-4 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:border-[#f97316] transition-all"
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-20 uppercase italic">
      <nav className="fixed top-0 w-full left-0 p-5 bg-white/80 backdrop-blur-md border-b border-slate-100 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
            <Link href="/" className="text-xl font-black italic tracking-tighter">CHASSIS <span className="text-[#e11d48]">PRO</span></Link>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-12 pt-28 md:pt-36 relative z-10">
        <header className="mb-12 border-l-8 border-[#e11d48] pl-6">
            <h1 className="text-3xl md:text-5xl font-extrabold text-slate-800 tracking-tight italic">Secure Checkout</h1>
            <p className="text-slate-400 text-[10px] mt-2 font-bold tracking-widest">Verify order and select delivery method</p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <section className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                <h3 className="text-xs font-bold text-[#e11d48] tracking-widest mb-8 flex items-center gap-3">
                    <span className="bg-[#e11d48] text-white w-6 h-6 flex items-center justify-center not-italic rounded-full text-[10px]">01</span>
                    Contact Information
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 not-italic">
                    ...
                    <div><label className={labelStyle}>WhatsApp Number</label><input onChange={e => setCustomer({...customer, whatsapp: e.target.value})} className={inputStyle} /></div>
            ...
            <section className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                <h3 className="text-xs font-bold text-[#e11d48] tracking-widest mb-8 flex items-center gap-3">
                    <span className="bg-[#e11d48] text-white w-6 h-6 flex items-center justify-center not-italic rounded-full text-[10px]">02</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 not-italic">
                    ...
                    <div><label className={labelStyle}>Postcode</label><input value={address.postcode} onChange={e => setAddress({...address, postcode: e.target.value})} className={`${inputStyle} text-[#f97316] text-xl font-black`} placeholder="43000" maxLength={5} /></div>
                    <div>
                        ...
                        {townSuggestions.length > 0 ? (
                            ...
                            <input value={address.town} onChange={e => setAddress({...address, town: e.target.value})} className={inputStyle} placeholder="MANUAL ENTRY" />
                        )}
                    </div>
            ...
            <section className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                <div className="flex justify-between items-center mb-8 border-b border-slate-50 pb-4">
                    ...
                    <span className="text-[10px] font-bold text-[#f97316] bg-orange-50 px-3 py-1 rounded-full border border-orange-100">Chargeable: {shippingData.weightInfo} KG</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 not-italic">
                    {shippingData.options.map(c => (
                        <div key={c.name} onClick={() => setSelectedCourier(c)} className={`p-6 border-2 rounded-2xl cursor-pointer transition-all ${selectedCourier?.name === c.name ? 'border-[#f97316] bg-orange-50 shadow-lg' : 'border-slate-100 bg-white hover:border-slate-300'}`}>
                            <p className="font-bold text-[10px] text-slate-400 uppercase italic">{c.name}</p>
                            <p className="font-black text-2xl text-slate-900 mt-2">RM {formatMoney(c.totalCost)}</p>
                            <p className="text-[9px] text-slate-400 font-bold mt-1 uppercase">{c.time}</p>
                        </div>
                    ))}
                    {shippingData.options.length === 0 && <p className="col-span-3 py-10 text-center text-slate-300 text-[10px] font-bold italic">ENTER POSTCODE TO SEE COURIERS</p>}
                </div>
            </section>
          </div>
          ...
          <div className="lg:sticky lg:top-32 h-fit">
            <div className="bg-white border border-slate-100 p-8 rounded-[2rem] shadow-2xl shadow-slate-200/60 space-y-8 relative overflow-hidden not-italic">
              ...
              <h2 className="text-xl font-black italic border-b border-slate-50 pb-4 tracking-tighter uppercase text-slate-900">Order Summary</h2>
              
              <div className="space-y-6 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {cart.map((item: any) => (
                    <div key={item.id} className="bg-slate-50/50 p-5 rounded-2xl border border-slate-100 relative group">
                        <div className="flex justify-between items-start mb-4 gap-4">
                            <div className="w-full">
                                <p className="font-black text-slate-900 text-sm leading-tight uppercase italic">{item.brand_name} - {item.name_en}</p>
                                <p className="text-slate-400 text-[9px] mt-1 font-bold uppercase italic">RM {formatMoney(item.price)} / UNIT</p>
                            </div>
                            <button onClick={() => removeItem(item.id)} className="text-slate-200 hover:text-[#e11d48] transition-colors">✕</button>
                        </div>
                        
                        <div className="flex items-center justify-between border-t border-slate-200 pt-4">
                            <div className="flex items-center bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                                <button onClick={() => updateQty(item.id, -1)} className="px-4 py-1.5 hover:bg-slate-50 text-slate-400 font-black text-lg">-</button>
                                <span className="px-4 py-1.5 text-xs font-black text-slate-900">{item.qty}</span>
                                <button onClick={() => updateQty(item.id, 1)} className="px-4 py-1.5 hover:bg-slate-50 text-slate-400 font-black text-lg">+</button>
                            </div>
                            <span className="font-black text-slate-900 text-sm tracking-tight">RM {formatMoney(Number(item.price || 0) * item.qty)}</span>
                        </div>
                    </div>
                ))}
              </div>
              ...
              <div className="space-y-3 pt-6 border-t border-slate-100">
                ...
                <div className="pt-8 mt-4 border-t-2 border-slate-900 flex justify-between items-center gap-4">
                    <span className="font-black text-xs italic text-slate-400 uppercase tracking-widest flex-shrink-0">Grand Total</span>
                    <span className="text-3xl md:text-4xl font-black text-[#e11d48] tracking-tighter italic leading-none truncate uppercase">
              ...
              <button 
                ...
                className="w-full bg-[#0f172a] text-white py-8 font-black uppercase italic tracking-[0.2em] hover:bg-[#e11d48] transition-all shadow-xl shadow-red-500/10 rounded-2xl text-sm disabled:opacity-20 active:scale-95"
              >
