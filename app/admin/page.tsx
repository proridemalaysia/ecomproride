    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans uppercase italic">
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/90 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-md">
            <div className="max-w-7xl mx-auto w-full bg-white shadow-2xl rounded-xl pb-10 relative mb-20 overflow-hidden flex flex-col min-h-[90vh] not-italic">
                <div className="sticky top-0 z-30 bg-white border-b p-6 md:p-10 flex justify-between items-center">
                    ...
                    <button onClick={() => {setShowAddForm(false); setIsEditing(false);}} className="text-slate-400 hover:text-[#e11d48] font-black text-xl transition-all">✕</button>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        {activeSubTab === 'spec' && (
                            <>
                                <div className="space-y-6 bg-slate-50 p-8 border border-slate-200 rounded-lg">
                                    <label className="text-[#f97316] font-black text-[10px] tracking-widest uppercase mb-4 block italic">Fitment</label>
                        ...
                        {activeSubTab === 'sales' && (
                            <div className="md:col-span-2 space-y-10">
                                <button type="button" onClick={() => setHasVariations(!hasVariations)} className={`px-12 py-4 font-black text-xs italic transition-all ${hasVariations ? 'bg-[#f97316] text-white shadow-lg' : 'bg-slate-300 text-slate-500'}`}>{hasVariations ? 'ON' : 'OFF'}</button>
                                {hasVariations ? (
                                    <div className="space-y-8 animate-in fade-in">
                                        ...
                                        <div className="overflow-x-auto border border-slate-200 rounded-xl shadow-inner">
                                            <table className="w-full min-w-[1000px] text-left">
                                                <thead className="bg-slate-50 text-[10px] font-black text-slate-400 border-b border-slate-200 uppercase">
                ...
                <div className="p-10 border-t border-slate-100 flex flex-col md:flex-row justify-end gap-6 bg-slate-50 mt-auto">
                    <button onClick={() => {setShowAddForm(false); setIsEditing(false);}} className="px-10 py-5 font-black uppercase text-xs text-slate-400 italic hover:text-[#e11d48]">Cancel</button>
                    {activeSubTab !== 'ship' ? (
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-12 uppercase italic">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 border-b-2 border-slate-100 pb-8 gap-6">
            <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter text-slate-900 leading-none uppercase">ADMIN <span className="text-[#e11d48]">HUB</span></h1>
            <button onClick={handleLogout} className="bg-[#0f172a] text-white px-10 py-3 text-[11px] font-black hover:bg-[#e11d48] transition-all rounded-md tracking-widest">Logout</button>
        </div>
