...
export default function AdminDashboard() {
  ...
  const fetchCarModels = async (brandId: string) => {
  ...
  const handleSaveProduct = async () => {
    ...
    if (p) {
        ...
        // FIXED: Optional chaining on pErr to satisfy Vercel TypeScript
        alert(pErr?.message || "An unknown database error occurred."); 
    }
    setLoading(false);
  }
  ...
  if (loading && !isAdmin) return <div className="min-h-screen bg-white flex items-center justify-center text-slate-900 font-black italic animate-pulse uppercase tracking-[0.3em]">Authorizing Hub...</div>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/90 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-md">
            <div className="max-w-6xl mx-auto w-full bg-white shadow-2xl rounded-xl pb-10 relative mb-20 overflow-hidden flex flex-col min-h-[90vh]">
                ...
                <div className="p-10 border-t border-slate-100 flex justify-end gap-6 bg-slate-50 mt-auto">
                    <button onClick={() => {setShowAddForm(false); setIsEditing(false);}} className="px-10 py-5 font-black uppercase text-xs text-slate-400 italic hover:text-[#e11d48] transition-colors">Cancel</button>
