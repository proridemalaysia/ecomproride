...
export default function AdminDashboard() {
  ...
  // --- DYNAMIC VARIATION STATE (UNLIMITED LEVELS) ---
  ...
  // --- MASTER FORM STATE (Controlled for persistence) ---
  ...
  const isTabComplete = (tab: string) => {
    switch(tab) {
        case 'basic': return formData.name_en && formData.name_bm && formData.category_id && formData.image_url;
        case 'spec': return formData.product_brand_id && formData.fit_vehicle_id;
        case 'desc': return formData.description_en && formData.description_bm;
        case 'sales': return hasVariations ? variantGrid.length > 0 && variantGrid.every(v => v.price_b2c > 0) : formData.price_b2c > 0;
        case 'ship': return formData.weight_kg > 0 && formData.length_cm > 0;
  ...
  // --- UNLIMITED VARIATION CALCULATOR ---
  useEffect(() => {
    if (hasVariations && !isEditing) {
        ...
        if (activeOptions.length > 0 && activeOptions[0].length > 0) {
            ...
            setVariantGrid(prevGrid => combos.map((combo) => {
                ...
                const existing = prevGrid.find(g => g.name === name);
                return {
                    name, attributes: combo,
  ...
  const handleSaveProduct = async () => {
    ...
    const first = variantGrid.find(v => v.is_active) || variantGrid[0];
    ...
    const payload = {
        ...
        has_variants: hasVariations, price_b2c: hasVariations ? first.price_b2c : formData.price_b2c, price_b2b: hasVariations ? first.price_b2b : formData.price_b2b,
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans uppercase italic">
      {showAddForm && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-md">
            <div className="max-w-7xl mx-auto w-full bg-white shadow-2xl rounded-xl pb-10 relative mb-20 overflow-hidden flex flex-col min-h-[90vh]">
                
                <div className="sticky top-0 z-30 bg-white border-b border-slate-100 p-6 md:p-10 flex justify-between items-center">
                    <h2 className="text-2xl font-black italic tracking-tighter text-slate-900">{isEditing ? 'Edit Listing' : 'New Listing'}</h2>
                        ...
                        {activeSubTab === 'spec' && (
                            <>
                                <div className="space-y-6 bg-slate-50 p-8 border border-slate-200 rounded-lg">
                                    <label className="text-[#f97316] font-black text-[10px] tracking-widest uppercase mb-4 block">Vehicle Fitment</label>
                            ...
                            <div className="md:col-span-2 space-y-10">
                                ...
                                {hasVariations && (
                                    <div className="space-y-8 animate-in fade-in">
                                        {variationLevels.map((level, lIdx) => (
                                            <div key={lIdx} className="bg-slate-50 border border-slate-200 p-8 relative rounded-lg">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                    <div><label className={labelStyle}>Level {lIdx+1} Name (e.g. Spec)</label><input className={inputStyle} value={level.name} onChange={e => {const n = [...variationLevels]; n[lIdx].name = e.target.value.toUpperCase(); setVariationLevels(n);}} /></div>
                                        ...
                                        {variationLevels.length < 5 && <button onClick={() => setVariationLevels([...variationLevels, { name: 'POSITION', options: ['FRONT'] }])} className="w-full border-2 border-dashed border-slate-200 p-4 text-[10px] font-black text-slate-400 hover:text-slate-900 transition-all uppercase">+ Add More Level</button>}
                                        <div className="overflow-x-auto border border-slate-200 rounded-xl">
                                            <table className="w-full min-w-[1000px] text-left">
                                                <thead className="bg-slate-50 text-[10px] font-black text-slate-400 border-b border-slate-200 italic uppercase">
                                                    <tr><th className="p-5">VARIANT COMBO</th><th className="p-5 text-[#f97316]">RETAIL RM</th><th className="p-5 text-[#e11d48]">DEALER RM</th><th className="p-5">SKU</th><th className="p-5">STOCK</th><th className="p-5 text-center">STATUS</th></tr>
                                                </thead>
                                                <tbody>
                                                    {variantGrid.map((v, i) => (
                ...
                <div className="sticky bottom-0 z-30 p-8 md:p-12 border-t border-slate-100 flex flex-col md:flex-row justify-end gap-6 bg-slate-50">
                    ...
                    {activeSubTab !== 'ship' ? (
                        <button 
                            ...
                            className={`px-24 py-6 font-black uppercase italic text-sm tracking-widest transition-all rounded-md shadow-xl ${isTabComplete(activeSubTab) ? 'bg-[#f97316] text-white hover:bg-[#0f172a]' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
                        >
                            Next Step →
                        </button>
                    ) : (
                        <button 
                            ...
                            disabled={!isTabComplete('ship')}
                            className={`px-24 py-6 font-black uppercase italic text-sm tracking-widest transition-all rounded-md shadow-xl ${isTabComplete('ship') ? 'bg-[#0f172a] text-white hover:bg-green-600' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
                        >
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-12">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 border-b-2 border-slate-100 pb-8 gap-6 uppercase italic">
            ...
            <div className="flex gap-4 w-full md:w-auto">
                <Link href="/products" className="flex-1 md:flex-none text-center border-2 border-slate-100 px-8 py-3 text-[11px] font-black hover:bg-slate-50 transition-all rounded-md">Shop</Link>
                <button onClick={handleLogout} className="flex-1 md:flex-none bg-[#0f172a] text-white px-10 py-3 text-[11px] font-black hover:bg-[#e11d48] transition-all rounded-md">Logout</button>
            </div>
        </div>
        ...
        {activeTab === 'products' && (
          <div className="space-y-10 not-italic">
            <div className="flex flex-col md:flex-row justify-between items-center bg-white p-12 border border-slate-200 rounded-2xl gap-8 shadow-sm">
                <div className="text-center md:text-left"><h2 className="text-3xl font-black italic text-slate-900 tracking-tighter uppercase italic leading-none">Warehouse Hub</h2><p className="text-sm text-slate-400 font-bold tracking-widest mt-2 uppercase italic">{products.length} Professional Records Active</p></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 italic uppercase">
                {products.map(p => (
                    <div key={p.id} className="bg-white border border-slate-200 p-8 hover:border-[#f97316] transition-all group flex flex-col justify-between shadow-xl relative overflow-hidden rounded-xl">
                        <div className="flex gap-6 items-start mb-8 italic">
                            ...
                            <div className="overflow-hidden">
                                ...
                                <p className="text-[10px] text-slate-400 mt-3 font-bold uppercase tracking-widest leading-none">{p.brand_name} SPECIALIST</p>
                                {p.has_variants && <p className="text-[9px] text-[#f97316] mt-2 font-black italic">✓ VARIATIONS ACTIVE</p>}
                            </div>
                        </div>
                        <div className="flex justify-between items-end border-t border-slate-100 pt-6">
                            <div><p className="text-[10px] text-slate-400 font-black tracking-widest uppercase mb-1 italic">Master Price</p><span className="text-slate-900 font-black text-xl tracking-tighter uppercase leading-none italic">RM{p.price_b2c?.toFixed(2)}</span></div>
                            <div className="flex gap-4">
                                <button onClick={() => handleEditClick(p)} className="text-[10px] text-[#f97316] font-black italic uppercase hover:text-slate-900">Edit</button>
                                <button onClick={async () => { if(confirm("Confirm Delete?")){ await supabase.from('products').delete().eq('id', p.id); fetchProducts() }}} className="text-[10px] text-slate-300 hover:text-[#e11d48] font-black italic transition-all uppercase leading-none">Delete</button>
                            </div>
                        </div>
                    </div>
                ))}
