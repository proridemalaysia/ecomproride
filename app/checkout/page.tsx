...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    if (!customer.name || !customer.whatsapp || !selectedCourier || !address.state) {
  ...
  // --- STYLING VARS ---
  const headerLabel = "text-sm font-bold text-slate-800 uppercase tracking-tight mb-2 block";
  const inputStyle = "w-full bg-white border border-slate-200 p-4 rounded-xl text-sm font-semibold text-slate-600 outline-none focus:border-blue-600 transition-all shadow-sm";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                <h3 className="text-xs font-bold text-blue-600 tracking-widest uppercase mb-8 italic">01. Select Your Free Gift</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className={`p-6 border-2 rounded-2xl transition-all ${hasFullSet ? 'border-slate-100 bg-white' : 'border-dashed border-slate-200 opacity-50'}`}>
                        ...
                        ) : ( <p className="text-xs font-bold text-slate-300 italic">Unlock with Full Set (4pcs)</p> )}
                    </div>
                </div>
            </section>
            ...
            {/* 02. VOUCHER */}
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                <h3 className="text-xs font-bold text-blue-600 tracking-widest uppercase mb-8 italic">02. Promotion Code</h3>
                <div className="flex gap-3">
                    ...
                    <button onClick={async () => {
                        ...
                        if (data) { setAppliedVoucher(data); alert("Applied!"); } else { alert("Invalid."); }
            ...
            {/* 03. SHIPPING DETAILS */}
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                <h3 className="text-xs font-bold text-blue-600 tracking-widest uppercase mb-8 italic">03. Shipping Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div><label className={headerLabel}>WhatsApp Number</label><input onChange={e => setCustomer({...customer, whatsapp: e.target.value})} className={inputStyle} placeholder="601..." /></div>
            ...
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                <div className="flex justify-between items-center mb-8 border-b pb-4">
                    <h3 className="text-xs font-bold text-blue-600 tracking-widest uppercase italic">04. Courier selection</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {shippingData.options.map(c => (
                        <div key={c.name} onClick={() => setSelectedCourier(c)} className={`p-6 border-2 rounded-2xl cursor-pointer transition-all ${selectedCourier?.name === c.name ? 'border-blue-600 bg-blue-50 shadow-md' : 'border-slate-100'}`}>
                            <p className="font-bold text-[10px] text-slate-400 uppercase tracking-tighter mb-2">{c.name}</p>
          ...
          <div className="lg:sticky lg:top-32 h-fit">
            <div className="bg-white border border-slate-200 p-8 rounded-[2.5rem] shadow-2xl space-y-8 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#e11d48] to-[#f97316]"></div>
              <h2 className="text-xl font-bold border-b pb-4 uppercase text-slate-900 italic">Order Total</h2>
                ...
                {cart.map((item: any) => (
                    <div key={item.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-100 relative group">
                        <div className="flex justify-between items-start gap-4">
                            <p className="font-bold text-slate-900 text-xs leading-tight italic uppercase">{item.name_en}</p>
                        ...
                        <div className="flex items-center justify-between mt-4 border-t border-slate-200 pt-4">
                            <div className="flex items-center bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                                <button onClick={() => updateQty(item.id, -1)} className="px-3 py-1 hover:bg-blue-600 hover:text-white font-black text-lg transition-all">-</button>
              ...
              <div className="space-y-3 pt-6 border-t-2 border-slate-100">
                {(wantsSticker || tshirtSize) && (
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-2 mb-4">
                        {wantsSticker && <p className="text-[10px] font-black text-blue-700 italic uppercase">✓ Proride Sticker</p>}
                        {tshirtSize && <p className="text-[10px] font-black text-blue-700 italic uppercase">✓ T-Shirt Size: {tshirtSize}</p>}
                    </div>
                )}
                <div className="flex justify-between text-xs font-bold text-slate-400"><span>Items</span><span className="text-slate-900">RM {formatMoney(itemsTotal)}</span></div>
                ...
                {appliedVoucher && <div className="flex justify-between text-xs font-bold text-green-600"><span>Voucher</span><span>- RM {formatMoney(discount)}</span></div>}
                <div className="pt-8 mt-4 border-t-2 border-slate-900 flex justify-between items-center gap-4">
                    <span className="font-bold text-xs text-slate-400 uppercase italic">Grand Total</span>
                    <span className="text-2xl md:text-4xl font-black text-blue-600 tracking-tighter leading-none">RM {formatMoney(grandTotal)}</span>
                </div>
              </div>
              <button onClick={handlePlaceOrder} disabled={loading || cart.length === 0 || !selectedCourier} className="w-full bg-[#0f172a] text-white py-7 font-bold uppercase italic hover:bg-blue-600 transition-all shadow-xl rounded-2xl active:scale-95 disabled:opacity-20">
