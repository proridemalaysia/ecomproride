...
export default function AdminDashboard() {
  ...
  const isTabComplete = (tab: string) => {
    ...
    if (tab === 'sales') return hasVariations ? variantGrid.length > 0 && variantGrid.every(v => v.price_b2c > 0) : formData.price_b2c > 0;
    ...
    if (hasVariations && !isEditing) {
        ...
        if (activeOptions.length > 0 && activeOptions[0].length > 0) {
            ...
            setVariantGrid(prevGrid => combos.map((combo) => {
                ...
                return { name, attributes: combo, price_b2c: existing?.price_b2c || 0, price_b2b: existing?.price_b2b || 0, sku: existing?.sku || '', stock: existing?.stock || 0, is_active: existing?.is_active ?? true };
            }));
        }
    }
  ...
  const handleSaveProduct = async () => {
    ...
    const selectedBrand = productBrands.find(b => b.id.toString() === formData.product_brand_id);
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans uppercase italic">
      {showAddForm && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-md">
            <div className="max-w-6xl mx-auto w-full bg-white shadow-2xl rounded-xl pb-10 relative mb-20 overflow-hidden flex flex-col min-h-[90vh] not-italic">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-12">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 border-b-2 border-slate-100 pb-8 gap-6 uppercase italic">
            ...
            <div className="flex gap-4 w-full md:w-auto">
                <Link href="/products" className="flex-1 md:flex-none text-center border-2 border-slate-100 px-6 py-3 text-[11px] font-black hover:bg-slate-50 transition-all rounded-md tracking-widest uppercase">Shop</Link>
                <button onClick={handleLogout} className="flex-1 md:flex-none bg-[#0f172a] text-white px-10 py-3 text-[11px] font-black hover:bg-[#e11d48] transition-all rounded-md uppercase tracking-widest">Logout</button>
            </div>
        </div>
                    ...
                    <div key={p.id} className="bg-white border border-slate-200 p-8 hover:border-[#f97316] transition-all group flex flex-col justify-between shadow-sm relative overflow-hidden rounded-xl">
                        ...
                        <div className="flex justify-between items-end border-t border-slate-100 pt-6">
                            ...
                            <div className="flex gap-4">
                                ...
                                <button onClick={async () => { if(confirm("Confirm removal?")){ await supabase.from('products').delete().eq('id', p.id); fetchProducts() }}} className="text-[10px] text-slate-300 hover:text-[#e11d48] font-black italic transition-all uppercase">Delete</button>
                            </div>
                        </div>
                    </div>
                ))}
