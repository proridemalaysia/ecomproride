...
export default function AdminDashboard() {
    ...
    async function init() {
        ...
        if (session) {
          ...
          if (profile?.role === 'ADMIN') {
            setIsAdmin(true)
            const [c, pb, vl, p, o, u] = await Promise.all([
  ...
  const handleEditClick = async (p: any) => {
    ...
    setEditId(p.id); setIsEditing(true); setShowAddForm(true); setActiveSubTab('basic'); setLoading(false);
  }
  ...
  const handleSaveProduct = async () => {
    ...
    if (p) {
        ...
        alert("DATABASE SYNCED."); setShowAddForm(false); setIsEditing(false); window.location.reload();
  ...
  if (loading && !isAdmin) return <div className="min-h-screen bg-white flex items-center justify-center text-slate-900 font-black italic animate-pulse">CHASSIS PRO HUB...</div>
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans uppercase italic">
      {showAddForm && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-md">
            <div className="max-w-7xl mx-auto w-full bg-white shadow-2xl rounded-xl pb-10 relative mb-20 overflow-hidden flex flex-col min-h-[90vh] not-italic">
                ...
                <div className="sticky top-[88px] z-20 bg-slate-50 flex gap-10 overflow-x-auto px-10 py-4 border-b">
                ...
                <div className="p-6 md:p-12 flex-1">
                    ...
                    {activeSubTab === 'basic' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            ...
                            <div className="space-y-6">
                                ...
                                <div><label className={labelStyle}>Gallery Links</label><textarea className={`${inputStyle} h-32 resize-none`} value={formData.gallery_input} onChange={e => setFormData({...formData, gallery_input: e.target.value})} /></div>
                            </div>
                        </div>
                    )}
                    {/* 02. SPEC */}
                    {activeSubTab === 'spec' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            ...
                            <div className="space-y-6">
                                <div><label className={labelStyle}>Part Manufacturer</label><select className={inputStyle} value={formData.product_brand_id || ''} onChange={e => setFormData({...formData, product_brand_id: e.target.value})}><option value="">-- SELECT --</option>{productBrands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
                    ...
                    {activeSubTab === 'desc' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                             <div><label className={labelStyle}>Long Description (EN)</label><textarea className={`${inputStyle} h-80 resize-none`} value={formData.description_en} onChange={e => setFormData({...formData, description_en: e.target.value})} /></div>
                             <div><label className={labelStyle}>Long Description (BM)</label><textarea className={`${inputStyle} h-80 resize-none`} value={formData.description_bm} onChange={e => setFormData({...formData, description_bm: e.target.value})} /></div>
                        </div>
                    )}
                    {/* 04. SALES (FIXED VARIATION UI) */}
                    {activeSubTab === 'sales' && (
                        <div className="space-y-10">
                            ...
                            {hasVariations ? (
                                <div className="space-y-10">
                                    ...
                                    {!isEditing && variationLevels.length < 5 && <button onClick={() => setVariationLevels([...variationLevels, { name: 'POSITION', options: ['FRONT'] }])} className="w-full border-2 border-dashed border-slate-200 p-5 text-[10px] font-black text-slate-400 hover:text-slate-900 transition-all uppercase italic">+ Add Level</button>}
                                            ...
                                            <tbody>
                                                {variantGrid.map((v, i) => (
                                                    <tr key={i} className={`border-b border-slate-100 ${!v.is_active ? 'opacity-30' : 'hover:bg-slate-50'}`}>
                                                        <td className="p-5 font-black text-xs italic uppercase text-slate-500">{v.name}</td>
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-12 uppercase italic">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 border-b-2 border-slate-100 pb-8 gap-6">
            ...
            <button onClick={handleLogout} className="bg-[#0f172a] text-white px-10 py-3 text-[11px] font-black rounded-md tracking-widest">Logout</button>
        </div>
        ...
        {activeTab === 'products' && (
          <div className="space-y-10 not-italic">
            <div className="flex flex-col md:flex-row justify-between items-center bg-white p-12 border border-slate-200 rounded-2xl gap-8 shadow-sm">
                <div className="text-center md:text-left"><h2 className="text-3xl font-black italic text-slate-900 tracking-tighter italic">Warehouse Hub</h2><p className="text-sm text-slate-400 font-bold mt-2 uppercase italic">{products.length} Records Sync</p></div>
                    ...
                    <div key={p.id} className="bg-white border border-slate-200 p-8 hover:border-[#f97316] transition-all group flex flex-col justify-between shadow-xl relative overflow-hidden rounded-xl">
                        <div className="flex gap-6 items-start mb-8 italic">
                            ...
                            <div className="overflow-hidden">
                                ...
                                {p.has_variants && <p className="text-[9px] text-[#f97316] mt-2 font-black italic border-l-2 border-[#f97316] pl-2">✓ VARIATIONS ACTIVE</p>}
                            </div>
                        </div>
                        <div className="flex justify-between items-end border-t border-slate-100 pt-6">
                            ...
                            <div className="flex gap-4">
                                <button onClick={() => handleEditClick(p)} className="text-[10px] text-[#f97316] font-black italic uppercase hover:text-slate-900 transition-colors">Edit</button>
