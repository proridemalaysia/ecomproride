...
export default function AdminDashboard() {
  ...
  const isTabComplete = (tab: string) => {
    ...
    if (tab === 'sales') return true; // Variations can be complex, allow next to check totals
  ...
  const fetchProducts = async () => { const { data } = await supabase.from('products').select('*, product_variants(*), product_fitment(vehicle_id)').order('id', { ascending: false }); setProducts(data || []) }

  // --- VARIATION ENGINE (Only auto-generates for NEW listings) ---
  useEffect(() => {
  ...
  // --- EDIT HANDLER (SYNCS ALL TABLES) ---
  const handleEditClick = async (p: any) => {
    setLoading(true);
    // 1. Fetch Fitment for this product
    const { data: fitment } = await supabase.from('product_fitment').select('vehicle_id').eq('product_id', p.id).single();
    const vehicle = vehicleMasterList.find(v => v.id === fitment?.vehicle_id);
  ...
  if (loading && !isAdmin) return <div className="min-h-screen bg-white flex items-center justify-center text-slate-900 font-black italic animate-pulse uppercase">CHASSIS PRO HUB SYNC...</div>
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans uppercase italic">
      {showAddForm && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 flex items-start justify-center p-4 md:p-10 overflow-y-auto backdrop-blur-md">
            <div className="max-w-7xl mx-auto w-full bg-white shadow-2xl rounded-xl pb-10 relative mb-20 flex flex-col min-h-[90vh] not-italic">
                ...
                <div className="sticky top-[88px] z-20 bg-slate-50 flex gap-4 md:gap-10 overflow-x-auto px-10 py-4 border-b">
                            ...
                            <div className="md:col-span-2 space-y-10">
                                ...
                                {hasVariations ? (
                                    <div className="space-y-8 animate-in fade-in">
                                        {/* Show generator controls ONLY if we are NOT editing existing variants */}
                                        {!isEditing && (
                                        ...
                                        {!isEditing && variationLevels.length < 5 && <button onClick={() => setVariationLevels([...variationLevels, { name: 'POSITION', options: ['FRONT'] }])} className="w-full border-2 border-dashed border-slate-200 p-4 text-[10px] font-black text-slate-400 uppercase italic transition-all hover:text-slate-900">+ ADD LEVEL</button>}
                                        ...
                                        <div className="overflow-x-auto border border-slate-200 rounded-xl">
                                            <table className="w-full min-w-[1000px] text-left">
                                                <thead className="bg-slate-50 text-[10px] font-black text-slate-400 border-b border-slate-200 italic uppercase">
                                                    <tr><th className="p-5">VARIANT COMBO</th><th className="p-5 text-[#f97316]">RETAIL RM</th><th className="p-5 text-[#e11d48]">DEALER RM</th><th className="p-5 text-center">SKU</th><th className="p-5 text-center">STOCK</th><th className="p-5 text-center">STATUS</th></tr>
                                                </thead>
                                                <tbody>
                                                    {variantGrid.map((v, i) => (
