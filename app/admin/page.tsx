    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans uppercase italic">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-12 uppercase italic">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 border-b-2 border-slate-100 pb-8 gap-6">
            <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter text-slate-900 leading-none uppercase leading-none">ADMIN <span className="text-[#e11d48]">HUB</span></h1>
            <button onClick={handleLogout} className="bg-[#0f172a] text-white px-10 py-3 text-[11px] font-black hover:bg-[#e11d48] transition-all rounded-md uppercase tracking-widest leading-none">Logout</button>
        </div>
        ...
        {activeTab === 'products' && (
          <div className="space-y-10 not-italic">
            <div className="flex flex-col md:flex-row justify-between items-center bg-white p-12 border border-slate-200 rounded-2xl gap-8 shadow-sm">
                <div className="text-center md:text-left uppercase italic"><h2 className="text-3xl font-black italic text-slate-900 tracking-tighter italic">Warehouse Hub</h2><p className="text-sm text-slate-400 font-bold mt-2 uppercase italic">{products.length} Records Synchronized</p></div>
                    ...
                    <div key={p.id} className="bg-white border border-slate-200 p-8 hover:border-[#f97316] transition-all group flex flex-col justify-between shadow-sm relative overflow-hidden rounded-xl">
                        ...
                        <div className="flex justify-between items-end border-t border-slate-100 pt-6">
                            ...
                            <div className="flex gap-4">
                                ...
                                <button onClick={async () => { if(confirm("Permanently delete?")){ await supabase.from('products').delete().eq('id', p.id); fetchProducts() }}} className="text-[10px] text-slate-300 hover:text-[#e11d48] font-black italic transition-all uppercase leading-none">Delete</button>
                            </div>
                        </div>
                    </div>
                ))}
