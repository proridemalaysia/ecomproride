...
export default function AdminDashboard() {
  ...
  // 8. --- UNIFIED VARIATION ENGINE (Always Dynamic) ---
  useEffect(() => {
    if (hasVariations) {
  ...
  const handleEditClick = async (p: any) => {
    ...
    // RECONSTRUCT Variation Levels from existing data
    if (p.has_variants && p.product_variants && p.product_variants.length > 0) {
        const firstAttr = p.product_variants[0].attributes;
        // In this architecture, attributes is an array of strings representing the choices for each level
  ...
  const handleSaveProduct = async () => {
    ...
    if (p) {
      ...
      alert("SUCCESS: HUB SYNCED."); setShowAddForm(false); setIsEditing(false); fetchProducts();
  ...
  if (loading && !isAdmin) return <div className="min-h-screen bg-white flex items-center justify-center text-slate-900 font-black italic animate-pulse uppercase tracking-widest">Authorizing Hub...</div>
                        ...
                        {activeSubTab === 'spec' && (
                            <>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div><label className={labelStyle}>Material</label><input className={inputStyle} value={formData.spec_material} onChange={e => setFormData({...formData, spec_material: e.target.value})} /></div>
                                </div>
                            </>
                        )}
                        {activeSubTab === 'desc' && (
                            ...
                            <div className="md:col-span-2 space-y-10">
                                ...
                                {hasVariations ? (
                                    <div className="space-y-8 animate-in fade-in">
                                        {/* UNLOCKED: Levels are now always visible and editable */}
                                        ...
                                        {variationLevels.length < 5 && <button onClick={() => setVariationLevels([...variationLevels, { name: 'POSITION', options: ['FRONT'] }])} className="w-full border-2 border-dashed border-slate-200 p-4 text-[10px] font-black text-slate-400 hover:text-slate-900 transition-all uppercase italic">+ Add Level</button>}
                    ...
                    <div key={p.id} className="bg-white border border-slate-200 p-8 hover:border-[#f97316] transition-all group flex flex-col justify-between shadow-sm relative overflow-hidden rounded-xl">
                        <div className="flex gap-6 items-start mb-8 italic">
                            ...
                            <div className="overflow-hidden">
                                ...
                                {p.has_variants && <p className="text-[9px] text-[#f97316] mt-2 font-black italic border-l-2 border-[#f97316] pl-2 leading-none">✓ VARIATIONS ACTIVE</p>}
                            </div>
                        </div>
